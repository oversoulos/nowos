...
function updateLight(ts) {
    ...
    if (isDay) {
        ...
        starEl.style.width  = sz + 'px';
        starEl.style.height = sz + 'px';
    } else {
    ...
    if (isDay) {
        const ang = 88 + dayP * 184;
        const str = (0.022 + arc * 0.062).toFixed(3);
    ...
    const starA = isDay
        ? Math.max(0, 1 - dayP * 7).toFixed(3)
        : Math.min(1, ngtP * 4).toFixed(3);
...
function initBlobs() {
    ...
    for (let i = 0; i < 14; i++) {
        BLOBS.push({
            x:   Math.random(),
            y:   0.28 + Math.random() * 0.72,
            rx:  0.06  + Math.random() * 0.19,
            ry:  0.032 + Math.random() * 0.10,
            spd: (0.11 + Math.random() * 0.36) * (Math.random() < 0.5 ? 1 : -1),
            ph:  Math.random() * Math.PI * 2,
            a:   0.028 + Math.random() * 0.046,
            dh:  Math.floor(Math.random() * 36) - 18,
            H,
        });
    }
}
...
el('s-flip').addEventListener('click', function() {
    if (!this.__drag) flip();
});
...
document.querySelectorAll('.socket').forEach(sock => {
    ...
    const down = e => {
        ...
        const move = e2 => {
            ...
            sock.style.right  = 'auto';
            sock.style.bottom = 'auto';
        };
        const up = () => {
